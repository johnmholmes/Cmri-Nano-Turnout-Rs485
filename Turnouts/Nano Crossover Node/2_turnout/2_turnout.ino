/*pins 0 - 1 are usedfor Rx and Tx no trigger pin required due to using
  ...
  it will also read 4 sensors/switchs
*/
#include <CMRI.h>
#include <Servo.h>
...
void loop() {
  cmri.process();
  turnout1 = (cmri.get_bit(0)); //turnout *001 jmri address
  turnout2 = (cmri.get_bit(1)); //turnout *002 jmri address

  if (turnout1 == 1 && t1state == 0) {
    turnout1Target = turnout1ThrownPosition;   
    t1state = 1;
    cmri.set_bit(0, HIGH); //*001 jmri address output
  }
  ...
  if (turnout2 == 1 && t2state == 0) {
    ...
    t2state = 1;
    cmri.set_bit(1, HIGH); //*002 jmri address output
  }
  ...
  digitalWrite (throughGreenLed, !cmri.get_bit(2)); 
  digitalWrite (throughRedLed, !cmri.get_bit(3)); 
  digitalWrite (divergingGreenLed, !cmri.get_bit(4)); 
  digitalWrite (divergingRedLed, !cmri.get_bit(5)); 
  digitalWrite (throughApproachGreenLed, !cmri.get_bit(6));  
  digitalWrite (throughApproachRedLed, !cmri.get_bit(7)); 
  digitalWrite (divergingApproachGreenLed, !cmri.get_bit(8)); 
  digitalWrite (divergingApproachRedLed, !cmri.get_bit(9)); 
  digitalWrite (sh13GreenLed, !cmri.get_bit(10)); 
  digitalWrite (sh13RedLed, !cmri.get_bit(11)); 
  digitalWrite (sh14GreenLed, !cmri.get_bit(12)); 
  digitalWrite (sh14RedLed, !cmri.get_bit(13)); 
